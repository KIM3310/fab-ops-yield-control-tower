...
def test_platform_health() -> None:
    ...
    export_proof_board = client.get("/api/export-proof-board")
    assert health.status_code == 200
    payload = health.json()
    assert payload["service"] == "semiconductor-ops-platform"
    assert payload["persistence"]["backend"] in {"jsonl", "sqlite"}
    ...
    assert export_payload["links"]["fab_ops_export_ledger"] == "/api/fab-ops/runtime/export-ledger"
    assert export_payload["domains"]["scanner"]["runtime_contract"] == "scanner-export-ledger-v1"
...
def test_fab_ops_health_and_service_grade_surfaces() -> None:
    ...
    export_ledger_payload = export_ledger.json()
    assert export_ledger_payload["schema"] == "fab-ops-export-ledger-v1"
...
def test_fab_ops_sensitive_routes_require_operator_token_when_enabled(monkeypatch, tmp_path: Path) -> None:
