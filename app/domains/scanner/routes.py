...
def _build_export_ledger() -> dict[str, Any]:
    ...
    return {
        ...
        "schema": "scanner-export-ledger-v1",
        "aws": aws_status(),
        "summary": {
...
def shift_handoff() -> dict[str, Any]:
    ...
    aws_s3 = export_handoff_to_s3(DOMAIN, payload["handoff_id"], payload)
    aws_ddb = persist_export_metadata_to_dynamodb(
        domain=DOMAIN,
        export_id=payload["handoff_id"],
        export_type="handoff_payload",
        payload=payload,
        summary={"headline": payload.get("headline"), "site": payload.get("site")},
    )
    aws_sqs = publish_event_to_sqs(DOMAIN, "handoff_export", {"handoff_id": payload["handoff_id"], "s3_key": (aws_s3 or {}).get("key")})
    return {"payload": payload, "aws_exports": {"s3": aws_s3, "dynamodb": aws_ddb, "sqs": aws_sqs}}
...
def shift_handoff_signature() -> dict[str, Any]:
    ...
    aws_s3 = export_handoff_to_s3(DOMAIN, signature["signature_id"], signature)
    aws_ddb = persist_export_metadata_to_dynamodb(
        domain=DOMAIN,
        export_id=signature["signature_id"],
        export_type="handoff_signature",
        payload=signature,
        summary={"algorithm": signature.get("algorithm"), "key_id": signature.get("key_id")},
    )
    aws_sqs = publish_event_to_sqs(DOMAIN, "handoff_signature_export", {"signature_id": signature["signature_id"], "s3_key": (aws_s3 or {}).get("key")})
    return {"payload": signature, "aws_exports": {"s3": aws_s3, "dynamodb": aws_ddb, "sqs": aws_sqs}}
...
def audit_feed() -> dict[str, Any]:
    ...
    aws_ddb = persist_export_metadata_to_dynamodb(
        ...
        payload={"items": AUDIT_EVENTS},
        summary={"events": len(AUDIT_EVENTS)},
    )
    aws_sqs = publish_event_to_sqs(DOMAIN, "audit_bundle_export", {"events": len(AUDIT_EVENTS), "s3_key": (aws_s3 or {}).get("key")})
